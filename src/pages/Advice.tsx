...
type AdvicePayload = {
  daily: Array<{ id: string; title: string; content: string; author: string; category: string; image: string }>;
  articles: Array<{ id: string; title: string; content: string; author: string; category: string; readTime: string; likes: number }>;
  challenges: Array<{ id: string; title: string; description: string; reward: string; participants: number; completed: boolean }>;
}

const Advice = () => {
  const [activeTab, setActiveTab] = useState('daily');
  const [completedChallenges, setCompletedChallenges] = useState<string[]>(['6']);
  const [advice, setAdvice] = useState<AdvicePayload>({ daily: [], articles: [], challenges: [] });
  const { t } = useLanguage();
      ...
      <div className="p-4 relative z-10">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="challenges" className="mt-6">
            <div className="space-y-4">
              {advice.challenges.map((challenge) => {
