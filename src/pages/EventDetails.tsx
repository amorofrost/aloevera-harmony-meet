...
// Note: attendees are currently IDs. If we need user details, fetch them separately via /api/users.
...
const EventDetails = () => {
  ...
  useEffect(() => {
    ...
    // TODO: load real join status from backend once available
    setIsJoined(['2'].includes(eventId));
  }, [eventId]);
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center relative">
        ...
        <div className="text-center relative z-10">
          <h2 className="text-xl font-bold mb-2">Загрузка события...</h2>
        </div>
      </div>
    );
  }
  ...
  const attendeeUsers: User[] = []; // Optionally map attendee IDs to users by fetching /api/users
  ...
  const handleGroupChatClick = () => {
    // Navigate to group chats filtered by event
